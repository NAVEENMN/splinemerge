"""Plot results from the diffusion experiment.

Reads merge_pde_diffusion.csv and generates a summary bar chart comparing
reconstruction RMSE and recovered diffusion coefficient across
experimental configurations.

Usage:
    python utils/plot_diffusion.py [path/to/merge_pde_diffusion.csv]

If no path is provided, reads from merge_pde_diffusion.csv in the current
working directory.
"""
...
def load_results(csv_path):
    """Load diffusion experiment results from CSV.

    Returns a list of dicts with keys: experiment, n_obs, noise,
    recon_rmse, D_hat, D_err, pde_rmse.
    """
    results = []
    with open(csv_path, 'r') as f:
        ...
        for row in reader:
            results.append({
                ...
                'D_err': float(row['D_err']),
                'pde_rmse': float(row['pde_rmse']),
            })
    return results


def plot_results(results, output_path='diffusion_summary.png'):
    """Generate a summary figure for the diffusion experiment results.
    ...
    d_errs = [r['D_err'] for r in results]
    ...
    # Right panel: Recovered D
    ax2.set_facecolor('#0d1117')
    bars2 = ax2.bar(x, d_hats, width, color='#c792ea', edgecolor='#30363d')
    ax2.axhline(y=D_TRUE, color='#ff6464', linewidth=1.5, linestyle='--',
                label=f'True D = {D_TRUE}')
    ax2.set_xlabel('Experiment', color='white')
    ax2.set_ylabel('Recovered D', color='white')
    ax2.set_title('Diffusion Coefficient Recovery', color='white',
                  fontweight='bold')
    ax2.set_xticks(x)
...
def main():
    if len(sys.argv) > 1:
        ...
        csv_path = 'merge_pde_diffusion.csv'

    if not os.path.exists(csv_path):
        print(f"ERROR: Results file not found: {csv_path}")
        print("Run experiment_diffusion.py first to generate results.")
        sys.exit(1)
